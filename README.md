...
| `STORAGE_BACKEND` | No | Defaults to `sqlite`. Use `supabase` for cloud persistence. |
| `SUPABASE_URL` | For Supabase | Supabase project URL. |
| `SUPABASE_SERVICE_ROLE_KEY` | For Supabase | Service role key for server-side inserts and reads. |
...
SQLite is the default local storage backend. For cloud runs, such as GitHub Actions, use Supabase so alert deduplication persists between runs.
...
Keep the service role key private. Do not commit it to Git.
